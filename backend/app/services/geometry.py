...
def _calc_h_bond_geometries_uncached(
    atoms: Atoms, distance_cutoff: float = 3.5, angle_cutoff: float = 120
...
def calc_h_bond_geometries(
    ...
    """Public entry point — caches by content fingerprint + cutoff params.

    Read-only contract: callers must not mutate the returned lists.
    Any change to atoms content naturally misses the cache.
    """
    return HBONDS_CACHE.get_or_compute(
        hbonds_cache_key(atoms, distance_cutoff, angle_cutoff),
        lambda: _calc_h_bond_geometries_uncached(atoms, distance_cutoff, angle_cutoff),
    )
...
def _get_bonds_and_ghosts_uncached(
    atoms: Atoms,
    bond_scale: float = 1.2,
    bond_overrides: Optional[Dict[str, str]] = None,
    bond_inference_mode: Literal["auto", "quick", "full"] = "auto",
    ...
    if ghost_bonds_data:
        ...
        for k in range(len(ghost_bonds_data)):
            ...
            if has_vectorized_intersections:
                ...
                # A near-zero-length intersection is NOT a failure: it means the
                # atom sits exactly on the face it bonds across (a fractional coord
                # of 0/1, ubiquitous in CIF crystals), so the stub legitimately
                # terminates AT the boundary. Accept it (it is culled below by the
                # `> 1e-8` length check) rather than falling back to a fixed-length
                # segment that overshoots OUTSIDE the cell. Only genuinely bad
                # intersections (non-finite, or absurdly long) keep the fallback.
                vectorized_u_valid = (
            ...
            # Guard against a stub that spans the whole bond across the cell.
            # The boundary intersection is normally correct and faithful: for a
            # clean cross-boundary bond the two stubs hit OPPOSITE faces and
            # together cover the bond split at the boundary (e.g. 0.33 + 0.67).
            # But when the neighbouring image lands exactly ON the far cell face
            # (ubiquitous in high-symmetry crystals) one side's intersection sits
            # at the full bond length (t == 1) while the other degenerates to zero
            # — a single full-bond stub spanning the cell. In that degenerate case
            # only, split the bond at its midpoint so neither side spans the cell
            # and both atoms emit a symmetric half-stub. The midpoint stays inside
            # the cell because it lies before the (t == 1) boundary crossing.
            full_bond_threshold = vec_norm * (1.0 - 1e-6)
            midpoint_u = starts_u[k] + direction * (vec_norm * 0.5)
            midpoint_v = starts_v[k] - direction * (vec_norm * 0.5)
            if float(np.linalg.norm(candidate_u_end - starts_u[k])) >= full_bond_threshold:
                candidate_u_end = midpoint_u
            if float(np.linalg.norm(candidate_v_end - starts_v[k])) >= full_bond_threshold:
                candidate_v_end = midpoint_v
...
def get_bonds_and_ghosts(
    ...
    Read-only contract: callers must not mutate the returned tuples/lists.
    Any change to atoms content naturally misses the cache.
    """
    if diagnostics is not None:
