...
class TestMaxUploadBytes:
    def test_default_when_unset(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("ATOMCANVAS_MAX_UPLOAD_MB", raising=False)
        assert _max_upload_bytes() == DEFAULT_MAX_UPLOAD_MB * 1024 * 1024

    def test_env_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("ATOMCANVAS_MAX_UPLOAD_MB", "10")
        assert _max_upload_bytes() == 10 * 1024 * 1024

    def test_invalid_falls_back_to_default(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setenv("ATOMCANVAS_MAX_UPLOAD_MB", "not-a-number")
        assert _max_upload_bytes() == DEFAULT_MAX_UPLOAD_MB * 1024 * 1024

    def test_nonpositive_falls_back_to_default(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setenv("ATOMCANVAS_MAX_UPLOAD_MB", "0")
        assert _max_upload_bytes() == DEFAULT_MAX_UPLOAD_MB * 1024 * 1024
...
def test_upload_rejects_oversized_file(
    ...
    """An upload larger than ATOMCANVAS_MAX_UPLOAD_MB is rejected with 413
    before any parsing, instead of being read unbounded into memory."""
    monkeypatch.setenv("ATOMCANVAS_MAX_UPLOAD_MB", "0.001")  # ~1 KB cap
    oversized = b"X" * 8192  # 8 KB > 1 KB cap

    response = client.post(
        "/api/structure/upload",
        files={"file": ("big.xyz", oversized, "text/plain")},
    )
