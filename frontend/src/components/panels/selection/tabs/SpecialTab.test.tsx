...
describe('SpecialTab', () => {
  ...
  it('describes fixed atoms in plain terms, without the old jargon', () => {
    render(<SpecialTab onSelect={onSelect} operation="replace" />);
    expect(screen.getByText(/frozen \/ fixed in place \(FixAtoms constraint\)/i)).toBeInTheDocument();
    // The de-jargoned copy no longer talks about "special properties".
    expect(screen.queryByText(/special properties/i)).not.toBeInTheDocument();
  });
});
