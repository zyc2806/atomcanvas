...
interface MinimalStyle {
    ...
    // visParams.bondRadius threaded explicitly — the single source of truth for
    // bond/ring sizing. BondStyleSettings no longer carries a radius field.
    bondRadius: number;
}
...
export function buildExportScene(
    ...
    const bondsGroup = buildBonds(structure, vis, style.bondRadius, colorForAtom, opacityForAtom, overrides, mat);
    if (bondsGroup) scene.add(bondsGroup);

    // --- Aromatic rings: a torus per ring under a single `rings` node ---
    const ringsGroup = buildRings(structure, vis, colorForAtom, style.bondRadius, mat);
