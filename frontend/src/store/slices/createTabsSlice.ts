...
export const createTabsSlice: StateCreator<StructureState, [], [], TabsSlice> = (set, get) => ({
  ...
  addTab: (doc, name) => {
    ...
    const tab: StructureTab = { id, name, doc, bondTopologyOverrides: {}, colorOverrides: null, opacityOverrides: null, radiusOverrides: null, perAtomColorOverrides: null, perAtomOpacityOverrides: null, elements: {}, camera: null };
    set((s) => ({
    ...
    // A frame index from the previous structure must not leak into this one
    // (which may have a shorter / no trajectory). Reset to frame 0, paused.
    get().resetPlayback();
    return id;
  },
  ...
  switchTab: (id) => {
    ...
    // Switching structures resets playback so a stale frame index can't apply to
    // the newly-active (possibly shorter) trajectory.
    get().resetPlayback();
  },
  ...
  closeTab: (id) => {
    ...
    set({ tabs, activeTabId: next?.id ?? null, topologyOverrides: next ? { ...next.bondTopologyOverrides } : {}, elements: next ? { ...next.elements } : {}, past: [], future: [] });
    ...
    // The closed tab's frame index must not survive onto the neighbor (or the
    // empty state); reset to frame 0, paused.
    get().resetPlayback();
  },
