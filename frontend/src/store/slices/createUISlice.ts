...
import { aromaticBondIds, toBondId } from '../../utils/aromaticBonds';
...
const buildSelectedBonds = (bonds: BondTuple[] | undefined, selectedSet: Set<number>): string[] => {
    ...
    // Aromatic bonds (order 1.5) are excluded from auto-selection — they must be
    // selected explicitly via toggleBondSelection (a direct bond click).
    const aromaticIds = aromaticBondIds(bonds);
    const selectedBondIds = new Set<string>();
    bonds.forEach(([u, v]) => {
        if (selectedSet.has(u) && selectedSet.has(v)) {
            const id = toBondId(u, v);
            if (!aromaticIds.has(id)) {
...
export const createUISlice: StateCreator<StructureState, [], [], UISlice> = (set, get) => ({
    ...
    setExporting: (isExporting) => set({ exporting: isExporting }),
    ...
    toggleSelection: (index) => set((state) => {
        ...
        if (isSelected) {
            ...
            if (state.structureData?.visualization?.bonds) {
                ...
                // Aromatic bonds (order 1.5) are not auto-added here — they must be
                // selected explicitly by clicking the bond directly.
                const aromaticIds = aromaticBondIds(bonds);
                ...
                newAtoms.forEach((otherIdx) => {
                    ...
                    if (bondLookup.has(bondId) && !aromaticIds.has(bondId)) {
    ...
    updateVisualization: async () => {
        const { structureData, visParams, topologyOverrides, activeTabId } = get();
        ...
        try {
            const data = await bondService.updateVisualization({
                ...
                bond_overrides: topologyOverrides || undefined,
