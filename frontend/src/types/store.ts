...
export interface UISlice {
    ...
    notification: { message: string; severity: 'success' | 'info' | 'warning' | 'error'; key: number } | null;
    ...
    // PNG export resolution multiplier (1x / 2x / 4x). Persistent setting read by
    // the export menu; threaded into the PNG capture as a supersampling factor.
    exportScale: number;
    setExportScale: (scale: number) => void;

    // Dedicated export-in-progress flag. Kept separate from `loading` so that
    // ViewerCanvas's camera-reset logic (which keys off `loading`) is not
    // spuriously triggered by an export.
    exporting: boolean;
...
export interface HistorySnapshot {
    ...
    // Per-element style preset map (PresetSlice.elements). Captured so that
    // "Reset all styles" and other per-element edits are undoable — without this
    // undo would restore per-atom overrides but lose per-element styling.
...
// Trajectory playback (T2b). Pure client-side frame-swapping over an already-loaded
// trajectory. This slice deliberately never calls pushHistory / updateStructure —
// scrubbing frames is a VIEWING action, not an edit, so it must not pollute undo.
export interface PlaybackSlice {
    currentFrame: number;   // 0 .. trajectory.length - 1
    isPlaying: boolean;
    fps: number;            // frames per second, default 10
    setCurrentFrame: (i: number) => void;   // clamped to [0, frameCount - 1]
    play: () => void;
    pause: () => void;
    togglePlay: () => void;
    stepFrame: (delta: number) => void;     // clamped at ends (no wrap)
    setFps: (fps: number) => void;
    resetPlayback: () => void;              // currentFrame = 0, isPlaying = false
}
...
export interface StructureTab {
    ...
    bondTopologyOverrides: Record<string, string>;   // "i-j" -> "delete" | "1.0" | "2.0" | ...
    colorOverrides: { [index: number]: string } | null;
    opacityOverrides: { [index: number]: number } | null;
    radiusOverrides: { [index: number]: number } | null;
    perAtomColorOverrides: { [index: number]: string } | null;
    perAtomOpacityOverrides: { [index: number]: number } | null;
    // Per-element style preset map (PresetSlice.elements). Captured per tab so
    // per-element styling does not leak / get lost across tab switches, mirroring
    // the per-atom override maps above.
    elements: Record<string, ElementStyle>;
...
export type StructureState = DataSlice & UISlice & SceneSlice & StyleSlice & HistorySlice & PresetSlice & TabsSlice & PlaybackSlice;
