...
describe('resolveShortcut', () => {
  ...
  it('maps Space to a playback toggle', () => {
    expect(resolveShortcut({ key: ' ' }, active)).toEqual({ type: 'playbackToggle' });
    expect(resolveShortcut({ key: 'Spacebar' }, active)).toEqual({ type: 'playbackToggle' });
  });

  it('maps ArrowLeft / ArrowRight to a playback step', () => {
    expect(resolveShortcut({ key: 'ArrowLeft' }, active)).toEqual({ type: 'playbackStep', delta: -1 });
    expect(resolveShortcut({ key: 'ArrowRight' }, active)).toEqual({ type: 'playbackStep', delta: 1 });
  });

  it('ignores playback keys while typing in a field', () => {
    expect(resolveShortcut({ key: ' ' }, typing)).toEqual({ type: 'none' });
    expect(resolveShortcut({ key: 'ArrowLeft' }, typing)).toEqual({ type: 'none' });
  });

  it('maps ? to toggleHelp when not in an editable target', () => {
    expect(resolveShortcut({ key: '?' }, active)).toEqual({ type: 'toggleHelp' });
  });
